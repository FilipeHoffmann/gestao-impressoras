...
class class_faturamento:
    ...
    def faturamento():
        query = f'''
        SELECT secretarias.nome, setores.nome_setor, impressoras.id_impressoras,impressoras.status, c.data_coleta, c.paginas_impressas,cotas.cota_mensal,produtos.valor_unitario
        ...
        for i in range(len(lista_contadores)):
            ...
            if (lista_contadores[i][5]-lista_contadores[i][6]) <= 0:
                ...
                lista_contadores[i].append(lista_contadores[i][7]*lista_contadores[i][8])
            custo_total += lista_contadores[i][9]
